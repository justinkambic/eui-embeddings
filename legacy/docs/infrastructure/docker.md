...
- Hardcoded in: `embed.py`, `frontend/pages/api/*.ts`, `mcp_server.py`, `scripts/index/index_eui_icons.py`, test scripts, documentation
...
- `scripts/index/index_eui_icons.py` - Ensure all service URLs use env vars
...
- Create `scripts/manage/manage-api-keys.sh`:
...
- Create `scripts/setup/setup-secrets.sh` for secret creation:
...
14. `scripts/setup/setup-secrets.sh` - Secret management script
15. `scripts/manage/manage-api-keys.sh` - API key management script
...
6. `scripts/index/index_eui_icons.py` - Update service URLs to use env vars
