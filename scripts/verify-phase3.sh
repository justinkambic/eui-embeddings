...
if [ -f "scripts/setup-https.sh" ]; then
    print_result "PASS" "scripts/setup-https.sh exists"
    if [ -x "scripts/setup-https.sh" ]; then
        print_result "PASS" "scripts/setup-https.sh is executable"
    else
        print_result "FAIL" "scripts/setup-https.sh is not executable"
    fi
else
    print_result "FAIL" "scripts/setup-https.sh missing"
fi
...
if [ -f "scripts/setup-https.sh" ]; then
    ...
    if grep -q "gcloud compute addresses" scripts/setup-https.sh; then
    ...
    if grep -q "ssl-certificates" scripts/setup-https.sh; then
    ...
    if grep -q "network-endpoint-groups" scripts/setup-https.sh; then
        print_result "PASS" "setup-https.sh includes NEG creation"
    else
        print_result "FAIL" "setup-https.sh missing NEG creation"
    fi
    
    if grep -q "forwarding-rules" scripts/setup-https.sh; then
